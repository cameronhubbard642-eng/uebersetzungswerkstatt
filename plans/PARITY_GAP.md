...
| §11.7 Deployment | 2 | 0 | 3 | 2 | 0 | 7 |
| §11.8 Configuration | 0 | 1 | 2 | 0 | 0 | 3 |
| **Totals** | **17** | **25** | **24** | **8** | **1** | **75** |
...
| 5 | iOS Safari update-detection hardening (`reg.waiting` check at registration, `reg.active` instead of `controller`, `updatefound` handler) | **DIVERGENT (intentional, recorded)** | *Intentional because:* iOS Safari produced spurious `reg.waiting` states with the same SW bytes on first load and every reload (Principal verification, symptom (a) on v16/v17). The Spanish-aligned lifecycle-state guards (`reg.waiting && reg.active` at registration time; `installed && reg.active` in the `statechange` handler) fired false-positive banners on every page load. German replaces both guards with client-side last-seen CACHE_NAME tracking via `GET_CACHE_NAME` postMessage + `uw_lastSeenCacheName` localStorage key. Banner shown only when the controlling SW's CACHE_NAME differs from `lastSeen`. Spanish retains the lifecycle-state approach; German diverges as a load-bearing fix. ARCHITECTURE.md §1.6 divergence 2. Landed WP-ARCH-G-3 Amendment 3. |
