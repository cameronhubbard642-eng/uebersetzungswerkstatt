...
| 3 | SW registered with `{ updateViaCache: 'none' }` and explicit `reg.update()` | **MATCH** | `index.html:24680` now calls `navigator.serviceWorker.register('sw.js', { updateViaCache: 'none' })`. Explicit `reg.update()` added at L24681 as the first statement after registration resolves. WP-DEP-G-2 (commit `0b813ee`). |
...
| 5 | iOS Safari update-detection hardening (`reg.waiting` check at registration, `reg.active` instead of `controller`, `updatefound` handler) | **MATCH** | `reg.waiting`-at-registration banner surface added at L24682 (WP-DEP-G-2, `0b813ee`). `navigator.serviceWorker.controller` replaced with `reg.active` in the `statechange` handler at L24686, eliminating the iOS PWA cold-launch race condition (Spanish `42e18fa` lesson). `updatefound` handler retained. |
...
|---|---|---|---|
| A1 | Install-time `self.skipWaiting()` (bypasses user consent for update) | **DIVERGENT (pending Principal confirmation)** *Intentional because:* commit `f79e45c` message states "with skipWaiting on install to force update". `sw.js:21`. Senior Dev flags for confirmation in Appendix B #B-4 — if reactive rather than intentional, score flips to a defect. |
