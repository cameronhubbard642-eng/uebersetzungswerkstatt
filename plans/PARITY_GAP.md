...
| 8 | Grammar mode: welcome + path roadmap, unit overview, lesson view with paradigm table, three exercise phases (Repetition / Cloze / Production), inline glossed thematic passages, due-review banner, review session | **PARTIAL** | Unit tree + lesson + exercise: MATCH. Three exercise phases + session demotions via `_thematicSessionDemotions` / `_thematicSessionFailures` + FSRS-driven phase selection via `_getEffectiveGrammarPhase()`: **MATCH** (landed WP-FE-G-18). Welcome + path roadmap: **MATCH** (landed WP-FE-G-20, 2026-04-24 — `renderGrammarWelcome()`, `getNextRecommendedLesson()`, `getGrammarLessonStatus()`; Gesamtfortschritt header + 10-unit path roadmap + Weiter CTA). Unit overview: **MISSING** (WP-FE-G-21). Inline glossed thematic passages: **MISSING**. Due-review banner: **MISSING** (closes under WP-FE-G-8). Review session is a stub (closes under WP-FE-G-8). |
