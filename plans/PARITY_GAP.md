...
| 8 | Grammar mode: welcome + path roadmap, unit overview, lesson view with paradigm table, three exercise phases (Repetition / Cloze / Production), inline glossed thematic passages, due-review banner, review session | **PARTIAL** | Unit tree + lesson + exercise: MATCH. Three exercise phases + session demotions via `_thematicSessionDemotions` / `_thematicSessionFailures` + FSRS-driven phase selection via `_getEffectiveGrammarPhase()`: **MATCH** (landed WP-FE-G-18). Welcome + path roadmap: **MATCH** (landed WP-FE-G-20, 2026-04-24). Unit overview: **MATCH** (landed WP-FE-G-21, 2026-04-24 — `renderGrammarUnitOverview()` with status badges + prereq hint). Stats panel (`renderGrammarStats`): **MATCH** (landed WP-FE-G-24, 2026-04-24 — German headers/`Fortschrittsübersicht`, `GRAMMAR_LESSONS` iteration). Inline glossed thematic passages: **MISSING**. Due-review banner: **MISSING** (closes under WP-FE-G-8). Review session is a stub (closes under WP-FE-G-8). |
