...
- **2026-04-24 — WP-FE-G-20 LANDED.** Port `renderGrammarWelcome()` + `getNextRecommendedLesson()` + `getGrammarLessonStatus()` from Spanish. Gesamtfortschritt header, 10-unit path roadmap, Weiter CTA. Old `_renderGrammarWelcome()` shim retired (body replaced with thin wrapper). `CACHE_NAME` v43→v44. `renderGrammarUnitOverview()` stubbed (WP-FE-G-21). LANDED: commit `aaef987` at `werkstatt-v44`.
